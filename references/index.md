...
└── cdp-protocol/               # Chrome DevTools Protocol 相关参考
    └── CDP 协议分析.md          # CDP 协议详细分析文档
```
...
| 项目路径 | 描述 | 版本/日期 |
|---------|------|----------|
| [cdp-protocol/](cdp-protocol/) | Chrome DevTools Protocol 官方仓库，包含协议定义、JSON Schema、PDL 文档等 | 1.3 / 2026-03 |
| [cdp-protocol/CDP 协议分析.md](cdp-protocol/CDP 协议分析.md) | CDP 协议详细分析文档，包含协议架构、核心命令、HTTP 端点、WebSocket 通信等 | 1.3 / 2026-03-09 |

**协议概述**:
Chrome DevTools Protocol 是一个基于 JSON 的协议，允许客户端与 Chrome/Chromium 浏览器进行通信，实现浏览器自动化、调试和分析功能。

**核心特性**:
- **HTTP REST API**: 用于浏览器发现、标签页管理 (`/json/version`, `/json/list`, `/json/new`)
- **WebSocket 通信**: 用于实时命令发送和事件接收
- **Domain 系统**: 按功能分组 (Target, Page, Runtime, DOM, Network, Input 等)
- **类型安全**: 完整的 JSON Schema 和 PDL (Protocol Definition Language) 定义

**协议架构**:
- **HTTP 端点**:
  - `GET /json/version` - 获取浏览器版本信息
  - `GET /json/list` - 获取所有标签页列表
  - `GET /json/new?url=...` - 创建新标签页
  - `POST /json/activate/:id` - 激活标签页
  - `POST /json/close/:id` - 关闭标签页
...
**核心 Domain**:
- **Target**: 目标管理 (创建、附加、关闭标签页)
- **Page**: 页面操作 (导航、截图、打印)
- **Runtime**: JavaScript 执行 (evaluate、callFunctionOn)
- **DOM**: DOM 操作 (querySelector、getOuterHTML)
- **Network**: 网络监控 (enable、getRequestResponseBody)
- **Input**: 输入事件 (dispatchMouseEvent、dispatchKeyEvent)
...
**分析文档内容**:
- 协议架构详解 (HTTP + WebSocket 双模式)
- 消息格式规范 (命令、响应、事件)
- 核心 Domain 和命令详解 (Target, Page, Runtime, DOM, Network, Input)
- HTTP 端点详细说明
- WebSocket 连接流程和会话管理
- 常见使用场景示例 (导航、JS 执行、截图、点击元素)
- 错误处理和错误码
- 安全考虑 (调试端口访问控制、认证机制)
- Kotlin 实现示例
...
- Chrome DevTools: https://developer.chrome.com/docs/devtools/protocol/
...
| 2026-03-09 | 更新 | 克隆 Chrome DevTools Protocol 官方仓库 |
| 2026-03-09 | 更新 | 创建 CDP 协议分析文档，包含完整的协议架构、核心命令和使用示例 |
