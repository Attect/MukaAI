...
data class ParameterSpec(
    val type: String, // string, number, boolean, file, etc.
...
class EchoSkillExecutor : SkillExecutor {
    override suspend fun execute(skillName: String, parameters: Map<String, String>): SkillExecutionResult {
        ...
        return SkillExecutionResult(
            success = true,
            output = "Echo: $text",
            metadata = mapOf("input_length" to text.length)
        )
    }
}
...
class CalculatorSkillExecutor : SkillExecutor {
    override suspend fun execute(skillName: String, parameters: Map<String, String>): SkillExecutionResult {
        ...
        // 简单的计算器实现 (仅用于演示)
        val result = try {
            // 注意：实际实现不应使用 eval，这里仅为演示
            val cleanedExpr = expression.replace(Regex("[^0-9+\\-*/(). ]"), "")
            evalExpression(cleanedExpr)
        } catch (e: Exception) {
            return SkillExecutionResult(
                success = false,
                output = "",
        ...
        return SkillExecutionResult(
            ...
            metadata = mapOf("result" to result)
        )
    }
...
- 验证加载流程
        ...
        exampleSkillFile.delete() // 清理临时文件
        println()
