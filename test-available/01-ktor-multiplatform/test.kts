...
@file:DependsOn("io.ktor:ktor-server-core:3.4.1")
@file:DependsOn("io.ktor:ktor-server-netty:3.4.1")
@file:DependsOn("io.ktor:ktor-server-cors:3.4.1")
@file:DependsOn("io.ktor:ktor-serialization-kotlinx-json:3.4.1")
@file:DependsOn("io.ktor:ktor-client-core:3.4.1")
@file:DependsOn("io.ktor:ktor-client-content-negotiation:3.4.1")
...
@Serializable
data class Response(val success: Boolean, val data: Any? = null, val error: String? = null)
...
fun main() = runBlocking {
    ...
    val server = embeddedServer(Netty, port = 8080) {
        ...
        routing {
            get("/health") {
                call.respond(Response(success = true, data = mapOf("status" to "ok")))
            }
            
            get("/api/message") {
                val msg = Message(text = "Hello from Ktor Server!")
                call.respond(Response(success = true, data = msg))
            }
            
            post("/api/message") {
                val msg = call.receive<Message>()
                println("收到消息：${msg.text}")
                call.respond(Response(success = true, data = mapOf("received" to msg.text)))
            }
        }
    }.start(wait = false)
