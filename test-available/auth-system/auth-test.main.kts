...
//> using deps "org.jetbrains.kotlinx:kotlinx-serialization-json:1.6.2"
...
class ServerTokenManager(
    ...
    private fun saveAuthorizedTokens() {
        val jsonArray = authorizedTokens.map { (token, clientId) ->
            buildJsonObject {
                put("token", token)
        ...
        authorizedTokensFile.writeText(Json.encodeToString(jsonArray))
...
class ClientTokenManager(
    ...
    fun initialize() {
        ...
        if (storageFile.exists()) {
            ...
            try {
                ...
                clientToken = json["clientToken"]?.let {
                    Json.decodeFromString<TokenData>(it.toString())
                }
    ...
    fun saveTokens() {
        val data = buildJsonObject {
            ...
            clientToken?.let {
                put("clientToken", Json.encodeToString(it))
            }
            put("clientId", clientId)
     ...
     * 设置预设 Token
     */
    fun setPresetToken(token: String) {
        presetToken = token
        saveTokens()
    }
     ...
     * 设置专属 Token
     */
    fun setClientToken(token: TokenData) {
        clientToken = token
        saveTokens()
    }
...
fun main() {
    ...
    clientManager.setPresetToken(userPresetToken)
    ...
    if (clientToken != null) {
        ...
        clientManager.setClientToken(clientToken)
    } else {
    ...
    client2Manager.setPresetToken(userPresetToken)
    ...
    if (client2Token != null) {
        ...
        client2Manager.setClientToken(client2Token)
    }
    println()
    ...
    if (newClientToken != null) {
        clientManager.setClientToken(newClientToken)
